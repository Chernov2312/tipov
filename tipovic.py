...
def Square_right():
    ...
    total_area = 0.0  # общая площадь

    for i in range(n):
        # Координата правого конца текущего участка
        x_right = a + (i + 1) * h

        # Высоты функций в правом конце участка
        f_val = func(x_right)
        g_val = func2(x_right)

            # Определяем высоту верхнего и нижнего прямоугольника
        upper_height = max(f_val, g_val)
        lower_height = min(f_val, g_val)

            # Вычисляем площадь текущего прямоугольника
        current_area = abs(upper_height - lower_height) * h

            # Суммируем площадь текущего прямоугольника
        total_area += current_area
    label(f"Площадь между графиками функций методом правых прямоугольников: {total_area:.4f}", 0, 50)
